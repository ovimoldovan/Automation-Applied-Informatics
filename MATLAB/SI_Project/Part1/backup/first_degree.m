...
for i=1:N2
    for j=N2:N
       f1(i,j-N2+1)=[theta(1).*X1(i) + theta(2).*X2(j)  + theta(3)];
       Y21(i,j-N2+1)=Y(i,j);
    end
end
for i=N2:N
    for j=1:N2
       f2(i-N2+1,j)=[theta(1).*X1(i) + theta(2).*X2(j)  + theta(3)];
       Y22(i-N2+1,j)=Y(i,j);
    end
end
for i=N2:N
    for j=N2:N
       f3(i-N2+1,j-N2+1)=[theta(1).*X1(i) + theta(2).*X2(j)  + theta(3)];
       Y23(i-N2+1,j-N2+1)=Y(i,j);
    end
end

size(f);
surf(x12,x21,f1,'EdgeColor','b');hold on;
